...
void TraCIDemoRSU11p::onBeacon(WaveShortMessage* wsm) {
    std::cout << "Receiving beacon from " << wsm->getSenderAddress()  << std::endl;
...
void TraCIDemoRSU11p::onData(WaveShortMessage* wsm) {
    ...
    annotations->scheduleErase(1,
            annotations->drawLine(wsm->getSenderPos(), mobi->getCurrentPosition(), "blue"));

    if (!sentMessage)
        sendMessage(wsm->getWsmData());
    std::cout << "receive Data from " << wsm->getSenderAddress() << std::endl;
}
