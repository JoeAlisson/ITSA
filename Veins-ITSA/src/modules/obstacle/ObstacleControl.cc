...
double ObstacleControl::calculateAttenuation(const Coord& senderPos,
    ...
    Coord bboxP1 = Coord(std::min(senderPos.x, receiverPos.x),
            std::min(senderPos.y, receiverPos.y));
    Coord bboxP2 = Coord(std::max(senderPos.x, receiverPos.x),
            std::max(senderPos.y, receiverPos.y));
    ...
    for (size_t col = fromCol; col <= toCol; ++col) {
        ...
        for (size_t row = fromRow; row <= toRow; ++row) {
            ...
            for (ObstacleGridCell::const_iterator k = cell.begin();
                    k != cell.end(); ++k) {
