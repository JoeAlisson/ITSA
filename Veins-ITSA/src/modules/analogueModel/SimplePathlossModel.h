...
public:
    ...
    SimplePathlossConstMapping(const DimensionSet& dimensions,
            SimplePathlossModel* model, const double distFactor);
...
public:
     ...
     */
    SimplePathlossModel(double alpha, double carrierFrequency, bool useTorus,
            const Coord& playgroundSize, bool debug) :
            pathLossAlphaHalf(alpha * 0.5), carrierFrequency(carrierFrequency), useTorus(
                    useTorus), playgroundSize(playgroundSize), debug(debug) {
     ...
     * method.
     */
    virtual double calcPathloss(const Coord& receiverPos,
