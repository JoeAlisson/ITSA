...
TraCIConnection* TraCIConnection::connect(const char* host, int port) {
    ...
    if (initsocketlibonce() != 0)
        opp_error("Could not init socketlib");
    ...
    *socketPtr = ::socket(AF_INET, SOCK_STREAM, 0);
    if (*socketPtr < 0)
        opp_error("Could not create socket to connect to TraCI server");

    if (::connect(*socketPtr, (sockaddr const*) &address, sizeof(address))
            < 0) {
        opp_error(
    ...
    {
        int x = 1;
        ::setsockopt(*socketPtr, IPPROTO_TCP, TCP_NODELAY, (const char*) &x,
                sizeof(x));
    }
...
TraCIBuffer TraCIConnection::query(uint8_t commandId, const TraCIBuffer& buf) {
    ...
    if (result == RTYPE_NOTIMPLEMENTED)
        opp_error(
                "TraCI server reported command 0x%2x not implemented (\"%s\"). Might need newer version.",
                commandId, description.c_str());
    if (result == RTYPE_ERR)
        opp_error(
                "TraCI server reported error executing command 0x%2x (\"%s\").",
                commandId, description.c_str());
    ASSERT(result == RTYPE_OK);
    return obuf;
}

TraCIBuffer TraCIConnection::queryOptional(uint8_t commandId,
        const TraCIBuffer& buf, bool& success, std::string* errorMsg) {
    sendMessage(makeTraCICommand(commandId, buf));
...
std::string TraCIConnection::receiveMessage() {
    if (!socketPtr)
        opp_error("Not connected to TraCI server");
    ...
    uint32_t msgLength;
    {
        ...
        while (bytesRead < sizeof(uint32_t)) {
            int receivedBytes = ::recv(socket(socketPtr),
                    reinterpret_cast<char*>(&buf2) + bytesRead,
                    sizeof(uint32_t) - bytesRead, 0);
            if (receivedBytes > 0) {
                bytesRead += receivedBytes;
            } else if (receivedBytes == 0) {
                opp_error(
                        "Connection to TraCI server closed unexpectedly. Check your server's log");
            } else {
                if (sock_errno() == EINTR)
                    continue;
                if (sock_errno() == EAGAIN)
                    continue;
                opp_error(
                        "Connection to TraCI server lost. Check your server's log. Error message: %d: %s",
                        sock_errno(), strerror(sock_errno()));
            }
        }
...
void TraCIConnection::sendMessage(std::string buf) {
...

    {
        ...
        while (bytesWritten < sizeof(uint32_t)) {
            size_t sentBytes = ::send(socket(socketPtr),
                    buf2.str().c_str() + bytesWritten,
                    sizeof(uint32_t) - bytesWritten, 0);
            if (sentBytes > 0) {
    ...
    {
        MYDEBUG << "Writing TraCI message of " << buf.length() << " bytes"
                       << endl;
        uint32_t bytesWritten = 0;
        while (bytesWritten < buf.length()) {
            size_t sentBytes = ::send(socket(socketPtr),
                    buf.c_str() + bytesWritten, buf.length() - bytesWritten, 0);
            if (sentBytes > 0) {
                bytesWritten += sentBytes;
            } else {
                if (sock_errno() == EINTR)
