...
const double MY_INFINITY = (
        std::numeric_limits<double>::has_infinity ? std::numeric_limits<double>::infinity() : std::numeric_limits<double>::max());
...
void PedestrianMobility::preInitialize(Client* con, const Coord& position, std::string road_id, double speed, double angle) {
    this->con  = con;
    this->lastUpdate = 0;
...
Coord PedestrianMobility::calculateAntennaPosition(const Coord& pedestrianPos) const {
    Coord corPos;
    if (antennaPositionOffset >= 0.001) {
        //calculate antenna position
        corPos = Coord(pedestrianPos.x - antennaPositionOffset * cos(-1), pedestrianPos.y + antennaPositionOffset * sin(-1), pedestrianPos.z);
    } else {
        corPos = Coord(pedestrianPos.x, pedestrianPos.y, pedestrianPos.z);
    }
    return corPos;
}
